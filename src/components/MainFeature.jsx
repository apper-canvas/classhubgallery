...
import { assignmentService, attendanceService, gradeService, studentService } from "../services";
...
const attendanceRecords = Array.from(selectedStudents).map(studentId => ({
        Name: `Attendance ${attendanceDate}`,
      ...
      const newAttendanceRecords = await attendanceService.create(attendanceRecords)
      if (newAttendanceRecords && newAttendanceRecords.length > 0) {
  ...
  const handleEnterGrade = async (e) => {
    ...
    try {
      const gradeData = {
        ...
        assignment_id: gradeForm.assignmentId,
        score,
        max_score: maxScore,
  ...
  return (
    ...
    <div className="mb-6">
        ...
        <Select
            ...
            <SelectTrigger
                className="w-full md:w-80 bg-white border-slate-300 rounded-xl shadow-sm hover:shadow-md transition-shadow">
            ...
            <SelectContent className="bg-white border-slate-200 rounded-xl shadow-education">
                {classes?.map(classItem => <SelectItem
                    key={classItem.id}
                    value={classItem.id}
    ...
    {selectedClass && <Tabs defaultValue="students" className="w-full">
        <TabsList
            className="grid w-full grid-cols-2 md:grid-cols-4 bg-slate-100 rounded-xl p-1 mb-6">
            <TabsTrigger
                value="students"
        ...
        <TabsContent value="students" className="mt-0">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                {/* Add Student Form */}
                <Card
                    className="lg:col-span-1 bg-gradient-to-br from-blue-50 to-indigo-50 border-blue-200 shadow-education">
                    <CardHeader>
                    ...
                    <CardContent>
                        <form onSubmit={handleCreateStudent} className="space-y-4">
                            ...
                            <Button
                                ...
                                {loading ? <ApperIcon name="Loader2" className="w-4 h-4 mr-2 animate-spin" /> : <ApperIcon name="Plus" className="w-4 h-4 mr-2" />}Add Student
                                                    </Button>
                        </form>
                    </CardContent>
                </Card>
                ...
                <Card className="lg:col-span-2 bg-white shadow-education">
                    ...
                    <CardContent>
                        <div className="space-y-3">
                            {classStudents.length === 0 ? <div className="text-center py-8 text-slate-500">
                                ...
                                return (
                                    <motion.div
                                        ...
                                        <div className="flex items-center space-x-2">
                                            ...
                                            <Badge
                                                variant="outline"
                                                className={`text-xs ${stats.averageGrade >= 80 ? "bg-green-50 text-green-700 border-green-200" : stats.averageGrade >= 60 ? "bg-yellow-50 text-yellow-700 border-yellow-200" : "bg-red-50 text-red-700 border-red-200"}`}>
        ...
        <TabsContent value="attendance" className="mt-0">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <Card
                    className="lg:col-span-1 bg-gradient-to-br from-green-50 to-emerald-50 border-green-200 shadow-education">
                    <CardHeader>
                    ...
                    <CardContent>
                        <form onSubmit={handleMarkAttendance} className="space-y-4">
                            ...
                            <div>
                                ...
                                <div
                                    className="max-h-40 overflow-y-auto space-y-2 bg-white rounded-lg border border-slate-300 p-3">
                                    {classStudents.map(student => <div key={student.id} className="flex items-center space-x-2">
                                        <Checkbox
                            ...
                            <Button
                                ...
                                {loading ? <ApperIcon name="Loader2" className="w-4 h-4 mr-2 animate-spin" /> : <ApperIcon name="Check" className="w-4 h-4 mr-2" />}Mark Present ({selectedStudents.size})
                                                    </Button>
                        </form>
                    </CardContent>
                </Card>
                ...
                <Card className="lg:col-span-2 bg-white shadow-education">
                    ...
                    <CardContent>
                        <div className="space-y-2 max-h-96 overflow-y-auto">
                            {localAttendance.filter(record => record.classId === selectedClass.id).sort((a, b) => new Date(b.date) - new Date(a.date)).slice(0, 20).map(record => {
                                ...
                                return (
                                    <div
                                        ...
                                        <div className="flex items-center space-x-3">
                                            <div
                                                className={`w-3 h-3 rounded-full ${record.status === "present" ? "bg-green-500" : "bg-red-500"}`}></div>
                                            <div>
        ...
        <TabsContent value="grades" className="mt-0">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                {/* Enter Grade Form */}
                <Card
                    className="lg:col-span-1 bg-gradient-to-br from-purple-50 to-violet-50 border-purple-200 shadow-education">
                    <CardHeader>
                        ...
                        <form onSubmit={handleEnterGrade} className="space-y-4">
                            <div>
                                ...
                                <Select
                                    ...
                                    onValueChange={value => setGradeForm(prev => ({
                                        ...prev,
                                        studentId: value
                                    }))}>
                                    ...
                                    <SelectContent className="bg-white border-slate-200 rounded-xl shadow-education">
                                        {classStudents.map(student => <SelectItem key={student.id} value={student.id}>
                                            {student.name}
                                        </SelectItem>)}
                                    </SelectContent>
                                </Select>
                            ...
                            <div>
                                ...
                                <Select
                                    ...
                                    <SelectContent className="bg-white border-slate-200 rounded-xl shadow-education">
                                        {assignments.filter(assignment => assignment.classId === selectedClass.id).map(assignment => <SelectItem key={assignment.id} value={assignment.id}>
                            ...
                            <Button
                                ...
                                {loading ? <ApperIcon name="Loader2" className="w-4 h-4 mr-2 animate-spin" /> : <ApperIcon name="Plus" className="w-4 h-4 mr-2" />}Enter Grade
                                                    </Button>
                        </form>
                    </CardContent>
                </Card>
                ...
                <Card className="lg:col-span-2 bg-white shadow-education">
                    ...
                    <CardContent>
                        <div className="space-y-2 max-h-96 overflow-y-auto">
                            {localGrades.filter(grade => {
                                const student = localStudents.find(s => s.id === grade.studentId);
                                return student && student.classId === selectedClass.id;
                            }).sort((a, b) => new Date(b.submittedDate) - new Date(a.submittedDate)).slice(0, 20).map(grade => {
                                const student = localStudents.find(s => s.id === grade.studentId);
                                ...
                                return (
                                    <div
                                        ...
                                        <div className="flex items-center space-x-3">
                                            <div
                                                className={`w-3 h-3 rounded-full ${percentage >= 90 ? "bg-green-500" : percentage >= 80 ? "bg-blue-500" : percentage >= 70 ? "bg-yellow-500" : percentage >= 60 ? "bg-orange-500" : "bg-red-500"}`}></div>
                                            <div>
                                                <div className="font-medium text-slate-800">{student?.name || "Unknown Student"}</div>
                                        ...
                                        <div className="text-right">
                                            ...
                                            <Badge
                                                variant="outline"
                                                className={`text-xs ${percentage >= 90 ? "bg-green-50 text-green-700 border-green-200" : percentage >= 80 ? "bg-blue-50 text-blue-700 border-blue-200" : percentage >= 70 ? "bg-yellow-50 text-yellow-700 border-yellow-200" : percentage >= 60 ? "bg-orange-50 text-orange-700 border-orange-200" : "bg-red-50 text-red-700 border-red-200"}`}>
                                                {percentage}%
                                                                              </Badge>
                                        </div>
                                    </div>
        ...
        <TabsContent value="overview" className="mt-0">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <Card
                    className="bg-gradient-to-br from-blue-500 to-blue-600 text-white shadow-education">
                    <CardHeader>
                        <CardTitle className="flex items-center justify-between">
                ...
                <Card
                    className="bg-gradient-to-br from-green-500 to-green-600 text-white shadow-education">
                    <CardHeader>
                        <CardTitle className="flex items-center justify-between">
                            ...
                            <div className="flex justify-between">
                                ...
                                <span className="font-bold">
                                    {classStudents.filter(
                                        s => calculateStudentStats(s).averageGrade < 60 && calculateStudentStats(s).averageGrade > 0
                                    ).length}
                                </span>
                            </div>
                        </div>
                ...
                <Card
                    className="bg-gradient-to-br from-purple-500 to-purple-600 text-white shadow-education md:col-span-2 lg:col-span-1">
                    <CardHeader>
                        <CardTitle className="flex items-center justify-between">
                    ...
                    <CardContent>
                        <div className="space-y-2">
                            ...
                            <div className="flex justify-between">
                                ...
                                <span className="font-bold">
                                    {classStudents.filter(
                                        s => calculateStudentStats(s).attendanceRate < 70 && calculateStudentStats(s).attendanceRate > 0
                                    ).length}
                                </span>
                            </div>
                        </div>
                                    ...
                                    return (
                                        <TableRow key={student.id} className="hover:bg-slate-50">
                                            ...
                                            <TableCell>
                                                <Badge
                                                    variant="outline"
                                                    className={`${stats.attendanceRate >= 90 ? "bg-green-50 text-green-700 border-green-200" : stats.attendanceRate >= 70 ? "bg-yellow-50 text-yellow-700 border-yellow-200" : "bg-red-50 text-red-700 border-red-200"}`}>
                                            ...
                                            <TableCell>
                                                <Badge
                                                    variant="outline"
                                                    className={`${stats.averageGrade >= 90 ? "bg-green-50 text-green-700 border-green-200" : stats.averageGrade >= 80 ? "bg-blue-50 text-blue-700 border-blue-200" : stats.averageGrade >= 70 ? "bg-yellow-50 text-yellow-700 border-yellow-200" : stats.averageGrade >= 60 ? "bg-orange-50 text-orange-700 border-orange-200" : stats.averageGrade > 0 ? "bg-red-50 text-red-700 border-red-200" : "bg-slate-50 text-slate-500 border-slate-200"}`}>
                                            ...
                                            <TableCell>
                                                <Badge
                                                    variant="outline"
                                                    className={`${stats.averageGrade >= 80 && stats.attendanceRate >= 90 ? "bg-green-50 text-green-700 border-green-200" : stats.averageGrade < 60 || stats.attendanceRate < 70 ? "bg-red-50 text-red-700 border-red-200" : "bg-yellow-50 text-yellow-700 border-yellow-200"}`}>
                                                    {stats.averageGrade >= 80 && stats.attendanceRate >= 90 ? "Excellent" : stats.averageGrade < 60 || stats.attendanceRate < 70 ? "At Risk" : "Good"}
                                                </Badge>
                                            </TableCell>
                                        </TableRow>
                                    );
    ...
    {!selectedClass && <div className="text-center py-12">
